...
public class ScanRequestCreationTest {
	...
	static {
		...
		try {
			...
			// Probably failed because we're running on Travis, not from Eclipse.
			pi.exec("sys.path.append('/home/travis/build/eclipse/"
				+	"org.eclipse.scanning/org.eclipse.scanning.command/scripts')");
			pi.exec("from mapping_scan_commands import *");
			pi.exec("from mapping_scan_commands import _instantiate");
		}
